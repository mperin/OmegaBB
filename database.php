<?php
/**
* @version $Id: database.php,v 1.11 2005/02/15 20:24:15 eddieajau Exp $
* @package Mambo
* @subpackage Database
...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db, $table_prefix ) {
		...
		function_exists( 'mysql_connect' ) or die( 'FATAL ERROR: MySQL support not available.  Please check your configuration.' );
 	($this->_resource = @mysql_connect( $host, $user, $pass )) or die( 'FATAL ERROR: Connection to database server failed. ( '.$host.',' . $user . ')' );
	
	//	($this->_resource = @mysql_connect( $host, $user, $pass )) or die( 'FATAL ERROR: Connection to database server failed.' );
		mysql_select_db($db) or die( "FATAL ERROR: Database not found. Operation failed with error: ".mysql_error());
