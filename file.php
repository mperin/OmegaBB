...
/*OmegaBB 0.9.2*/
    ...
    if ($file_id) { 
		...
		if (($settings->avatars_allowed == 0) && (!isMod($user_id))) {header("HTTP/1.0 404 Not Found"); exit;}
		...
		if (($settings->file_upload_allowed == 0) && (!isMod($user_id))) {header("HTTP/1.0 404 Not Found"); exit;}
